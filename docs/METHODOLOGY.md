...
2. **Verify session config before collecting data** (pre-flight):
   ...
   - **Permission mode: record it, and switch to DEFAULT before any matcher row (mandatory as of 2026-09-02).** CC 2.1.258 sessions start in **auto mode** (observed even under `--setting-sources project` with zero user settings), and auto mode's classifier silently ALLOWs every matcher-prompt family — Family-2, Family-3, chains, allow-gaps alike — self-attributed by an "Allowed by auto mode classifier" tool-result line. A probe run in auto mode measures the classifier, not the matcher. Switch with shift+tab and confirm the mode indicator before row 1. Auto-mode sweeps are a legitimate *second* condition when the classifier itself is the target. See FINDINGS 2026-09-02.
